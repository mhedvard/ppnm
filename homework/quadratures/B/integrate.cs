...
public class integrate{	
	...
	public static double ccIntegrate(Func<double, double> f, double a, double b, double d = 0.001, double e = 0.001){
		Func<double,double> g;
		if(a==-1 && b == 1)  g = t => f(Cos(t))*Sin(t); 
		else g = t => f((a+b)/2+(b-a)/2*Cos(t))*Sin(t)*(b-a)/2; 
		return Integrate(g,0,PI); 
	}
} 
